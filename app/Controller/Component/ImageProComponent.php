 ...
 * @package default 
 */  
class ImageProComponent extends Component { 
    /* 
     * @var array 
     * @access private 
     *  
     ...
     */ 
    public $config_document_root = WWW_ROOT;      
    /* 
     * @var int 
     * @access public 
     *  
     ...
     */ 
    public function uploaded_image_url($link = null) { 
		if(@GetImageSize($link)){
			$image_info = explode('/',$link);
			$filename = $image_info[count($image_info)-1];
			if(file_put_contents($this->save_paths['upload'].$filename, file_get_contents($link))){
				return $filename;
			}else{
				return false;
			}
    ...
    public function thumb($file) { 
        ...
        $destination = $this->save_paths['thumb'] .  $pathinfo['filename'] .'_'. $this->width .'x'. $this->height . '.' . $pathinfo['extension'];			
        /* 
		...
		$phpThumb->config_document_root = $this->config_document_root;			
